...
void next_state_tile(BitVect* gameState, int width, int height, int row, int col, 
        BitVect* marked)
{
    int numNeighbours = 0;
    ...
    for (int i = row - 1; i <= row + 1; i++) {
        ...
        for (int j = col - 1; j <= col + 1; j++) {
            ...
            if (bit_vect_get(gameState, checkPos))
                numNeighbours++;
    ...
    if (bit_vect_get(gameState, curPos)) {
        ...
        if (numNeighbours < 2 || numNeighbours > 3)
            //marked[curPos] = true;
            bit_vect_set(marked, curPos, true);
            //gameState[curPos] = ' ';
    } else {
        if (numNeighbours == 3)
            //marked[curPos] = true;
...
{
    BitVect marked;
    bit_vect_init(&marked, width * height);
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            //if (marked[curPos]) 
            if (bit_vect_get(&marked, curPos)) 
                //gameState[curPos] = (gameState[curPos] != ' ') ? ' ' : '#';
                bit_vect_comp(gameState, curPos);
        }
    }
}
